//#32 { retosparaprogramadores } - BATALLA ENTRE DEADPOOL Y WOLVERINE
/*
 * EJERCICIO:
...
if (isBrowser) {
 ...
 body {
    ...
    // Create the first combatant (Wolverine)
            ...
            div1.classList.add('contrincant1');
            ...
            div2.classList.add('contrincant2');
            ...
            title.textContent = 'Retosparaprogramadores #32.';
            title.style.setProperty('font-size', '3.5vmax');
            title.style.setProperty('color', '#fff');
            title.style.setProperty('line-height', '10vh');
            document.body.appendChild(title);

}else {
    log('This code is designed to run in a browser environment. Skipping window-related code.'); 
    log('Retosparaprogramadores #32');
}
...
class Combatant {
    ...
    constructor(
        ...
        this.attackPotence = attackDamageMax;
    }
}
...
class Battle {
    ...
    constructor(c1: Combatant, c2: Combatant) {
        this.combatant1 = c1;
        this.combatant2 = c2;
        this.result = '';
    ...
    attack(attacker: Combatant, defender: Combatant) {
        let avoidAttack = Math.floor(Math.random() * 101);
        let div1, div2;
        if (isBrowser){
            div1 = document.querySelector('.contrincant1')    
            div2 = document.querySelector('.contrincant2')
        }

        if (avoidAttack < 101 - defender.defense) {
            const maxDamage = Math.min(attacker.attackDamageMax, attacker.attackPotence);
            const minDamage = attacker.attackDamageMin;
            const damage = Math.floor(Math.random() * (maxDamage - minDamage + 1)) + minDamage;

            defender.life -= damage;
            attacker.attackPotence -= 10;

            if (isBrowser && logContainer) {
                logContainer.innerHTML += `<p>${attacker.name} dealt ${damage} damage to ${defender.name}. ${defender.name} has ${defender.life} life left.</p>`;
            } else {
                console.log(`${attacker.name} dealt ${damage} damage to ${defender.name}. ${defender.name} has ${defender.life} life left.`);
            }

            if (defender.life <= 0) {

                if(isBrowser && defender.name !== 'Wolverine'){
                    div2?.classList.add('loser'); 
                    div1?.classList.add('winner');
                }else if(isBrowser && defender.name === 'Wolverine'){
        ...
        if (attacker.attackPotence < attacker.attackDamageMin) {
            attacker.attackPotence = attacker.attackDamageMin;
        }
    }

    declareWinner(winner: Combatant, loser: Combatant) {
        if (isBrowser && logContainer) {
    ...
    round() {
        if (this.combatant1.life > 0 && this.combatant2.life > 0) {
            ...
            if (this.combatant1.life <= 0 && this.combatant2.life <= 0) {
                ...
                const div2 = document.querySelector('.contrincant2');
                const div1 = document.querySelector('.contrincant1');
                if (div2 && div1) {
                    div2.classList.add('winner');
                    div1.classList.add('loser');
                }
