...
// Note: 
/*  **Shebang (`#!/usr/bin/env`)**: Instead of specifying a fixed path to the Node.js
  interpreter (such as `/usr/local/bin/node`), `env` is used to search for `node` in the
  current execution environment. This makes the script more portable, as it does not depend 
 on the exact location of Node.js. */

//#33 { retosparaprogramadores } - RESCATANDO A MICKEY
/*
 * EJERCICIO:
...
const obstacle: string = '\u2B1B'; // ⬛️ Obstacle
const emptySpace: string = '\u2B1C'; // ⬜️ Empty space
const mickey: string = '\u1F42D'; // 🐭 Mickey
const exit: string = '\u1F6AA'; // 🚪 Exit
const path: string = '\u1F40E'; // 🐊 Path
...
let position: Position = { x: 0, y: 0 }; // Start position for Mickey

// Function to generate a random maze with obstacles
function generateRandomMaze(size: number): string[][] {
    const maze: string[][] = Array.from({ length: size }, () => Array(size).fill(obstacle)); // ⬛️ Obstacle
    ...
    // Ensure the start and exit positions are set correctly
...
function dijkstra(maze: string[][], recursive: boolean = false): Position[] {
    const size: number = maze.length;
    ...
    const previous: (Position | null)[][] = Array.from({ length: size }, () => Array(size).fill(null));

    distances[0][0] = 0; // Distance from the entrance is 0

    const queue: { x: number; y: number; distance: number }[] = [{ x: 0, y: 0, distance: 0 }];
    ...
    while (queue.length > 0) {
        ...
        for (const { x: dx, y: dy } of directions) {
            ...
            if (newX >= 0 && newX < size && newY >= 0 && newY < size) {
                const newDistance: number = distance + (maze[newY][newX] === obstacle ? 10 : 1); // If it's an obstacle, add 10 to the distance
    ...
    while (x !== 0 || y !== 0) {
        ...
        path.push({ x, y });
        const prev: Position = previous[y][x]!;
        x = prev.x;
        y = prev.y;
    }
...
function displayMaze(maze: string[][]): void {
    console.log('\nMaze:\n');
    maze.forEach(row => {
...
function moveMickey(dir: string, maze: string[][]): boolean {
...
rl.question('Enter the size of the maze (between 7 and 15): ', (input: string) => {
    const mazeSize: number = parseInt(input);
    ...
    const maze: string[][] = generateRandomMaze(mazeSize);
    
    // Carve a path in the maze using Dijkstra's algorithm
    const path: Position[] = dijkstra(maze);
    ...
    function startGame(): void {
        rl.question("Choose a direction to move (up - w, down - s, left - a, right - d): ", (answer: string) => {
            const moveSuccessful: boolean = moveMickey(answer.toLowerCase(), maze);
